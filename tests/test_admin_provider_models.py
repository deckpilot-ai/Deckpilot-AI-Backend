...
def test_llm_priority_routing_cascade(db_session: Session):
    import asyncio
    ...
    ProviderRouter.save_provider_models(
        ...
        models_data=[
            {"model_id": "alpha-model-top", "display_name": "Alpha Top", "priority": 90, "enabled": 1},
            {"model_id": "alpha-model-backup", "display_name": "Alpha Backup", "priority": 50, "enabled": 1},
        ],
    )
    ...
    async def _run():
        async def mock_post(url, *args, **kwargs):
            ...
            # Fail on alpha-model-top with 500 error to test failover to alpha-model-backup
            if model == "alpha-model-top":
                ...
                resp_body = {
                    "choices": [
                        {
                            "message": {
                                "content": '{"plan": "Generated by Alpha Backup"}'
                            }
                        }
                    ]
                }
...
def test_admin_model_diagnostics_and_test_all(client: TestClient, db_session: Session):
    token = get_admin_token(client, db_session)
    headers = {"Authorization": f"Bearer {token}"}
    ...
    create_resp = client.post(
        ...
        json={
            ...
            "models": [
                {"model_id": "diag-model-1", "display_name": "Diag Model 1", "priority": 90, "enabled": 1},
                {"model_id": "diag-model-2", "display_name": "Diag Model 2", "priority": 80, "enabled": 1},
            ],
        },
    )
    assert create_resp.status_code == 201
    ...
    # Get model DB IDs
    models_resp = client.get(f"/api/v1/admin/providers/{prov_id}/models", headers=headers)
    assert models_resp.status_code == 200
    models = models_resp.json()
    assert len(models) == 2

    # Mock httpx POST for diagnostic ping
    async def mock_ping(url, *args, **kwargs):
        json_body = kwargs.get("json") or {}
        model = json_body.get("model")
        if model == "diag-model-1":
            return httpx.Response(
                200,
                json={"choices": [{"message": {"content": "OK"}}], "usage": {"total_tokens": 5}},
                request=httpx.Request("POST", url),
            )
        elif model == "diag-model-2":
            return httpx.Response(
                401,
                text="Unauthorized model access",
                request=httpx.Request("POST", url),
            )
        return httpx.Response(404, text="Not Found", request=httpx.Request("POST", url))

    with patch("httpx.AsyncClient.post", side_effect=mock_ping):
        # 1. Test single model by db ID
        m1_id = models[0]["id"]
        test_m1 = client.post(
            f"/api/v1/admin/providers/{prov_id}/models/{m1_id}/test",
            headers=headers,
        )
        assert test_m1.status_code == 200
        data1 = test_m1.json()
        assert data1["success"] is True
        assert data1["model_id"] == "diag-model-1"
        assert data1["status_code"] == 200
        assert "latency_ms" in data1

        # 2. Test single model by string model_id
        test_m_str = client.post(
            f"/api/v1/admin/providers/{prov_id}/test-model",
            headers=headers,
            json={"model_id": "diag-model-1"},
        )
        assert test_m_str.status_code == 200
        assert test_m_str.json()["success"] is True

        # 3. Test All Models on provider
        test_all = client.post(
            f"/api/v1/admin/providers/{prov_id}/test-all",
            headers=headers,
        )
        assert test_all.status_code == 200
        all_data = test_all.json()
        assert all_data["total_tested"] == 2
        assert len(all_data["results"]) == 2
        # Check that one succeeded and one failed as mocked
        res_map = {r["model_id"]: r for r in all_data["results"]}
        assert res_map["diag-model-1"]["success"] is True
        assert res_map["diag-model-2"]["success"] is False
